...
export default {
  ...
  methods: {
      ...
      verifyPhoneCode(){
          ...
          Utils.verifyPhoneCode(self, data, Utils.REQUEST_FINDPWD).then(function(res){
      ...
      sendEmail(block){
          ...
          Utils.sendEmailRequest(self, data).then(function(res){
              ...
              localStorage.setItem('tv_account_byemail', self.user.account); //把邮箱种到本地存储中：作用 当点击立即进入邮箱按钮进入邮箱登录页再返回时，页面中的邮箱已被清空，此时从缓存中读取刚才输入的邮箱
          },function(res){
              self.errmsg= Utils.showErrorMsg(res);
